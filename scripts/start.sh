...
have pnpm || { c_step "Installing pnpm"; corepack enable >/dev/null 2>&1 || true; corepack prepare pnpm@10.33.0 --activate >/dev/null 2>&1 || npm install -g pnpm@10; }
have pnpm || { c_err "Could not install pnpm. Run: npm install -g pnpm"; exit 1; }
...
( pnpm dev:api >"$API_LOG" 2>&1 ) & API_PID=$!
for i in $(seq 1 90); do curl -fsS -o /dev/null http://localhost:4000/api/v1/attendance/mobile/config 2>/dev/null && break; sleep 1; done
curl -fsS -o /dev/null http://localhost:4000/api/v1/attendance/mobile/config 2>/dev/null || { c_err "The API did not start. Log: $API_LOG"; tail -20 "$API_LOG"; exit 1; }
c_ok "API ready on http://localhost:4000 (API reference at /docs)"

c_step "Starting the web app"
if [ "$HTTPS" = 1 ]; then
  ( cd apps/web && pnpm exec next dev -p 3000 -H 0.0.0.0 --experimental-https >"$WEB_LOG" 2>&1 ) & WEB_PID=$!
  SCHEME=https
else
  ( pnpm dev:web >"$WEB_LOG" 2>&1 ) & WEB_PID=$!
  SCHEME=http
fi
for i in $(seq 1 90); do curl -fsSk -o /dev/null "$SCHEME://localhost:3000/login" 2>/dev/null && break; sleep 1; done
c_ok "Web app ready"
...
cat <<BANNER
  ...
  ────────────────────────────────────────────────────────────────────
   Open:        $SCHEME://localhost:3000
   API docs:    http://localhost:4000/docs
$( [ "$HTTPS" = 1 ] && [ -n "$LAN_IP" ] && echo "   From a phone on the same Wi-Fi:  https://$LAN_IP:3000/attendance
...
if have open; then open "$SCHEME://localhost:3000" >/dev/null 2>&1 || true
elif have xdg-open; then xdg-open "$SCHEME://localhost:3000" >/dev/null 2>&1 || true; fi
