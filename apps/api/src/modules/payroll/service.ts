...
export async function calculateRun(db: Kysely<DB>, runId: string): Promise<{ employees: number; totals: { gross: number; earnings: number; deductions: number; net: number }; exceptions: number }> {
  ...
  for (const e of employees) {
    ...
    else if (t.scheduled_days === 0 && t.week_off_days === 0 && t.holiday_days === 0) exList.push('NO_ATTENDANCE_DATA: no processed attendance for this period — verify shift assignment and run attendance processing');
