...
export const jobRoutes: FastifyPluginAsync = async (app) => {
  ...
  r.get('/titles', { preHandler: requirePermission('jobs:read', 'org:read'), schema: { tags: ['jobs'], summary: 'Job titles with their family / function / default grade / career level links', response: { 200: z.array(z.object({ id: z.string(), code: z.string(), title: z.string(), grade: z.string().nullable(), jobFamilyId: z.string().nullable(), jobFamilyName: z.string().nullable(), jobFunctionId: z.string().nullable(), jobFunctionName: z.string().nullable(), defaultGradeId: z.string().nullable(), defaultGradeCode: z.string().nullable(), careerLevelId: z.string().nullable(), careerLevelCode: z.string().nullable(), employees: z.number() })) } } }, async () =>
    (await app.db.selectFrom('designations as d').leftJoin('job_families as f', 'f.id', 'd.job_family_id').leftJoin('job_functions as fn', 'fn.id', 'd.job_function_id').leftJoin('grades as g', 'g.id', 'd.default_grade_id').leftJoin('career_levels as l', 'l.id', 'd.career_level_id')
      .select(['d.id', 'd.code', 'd.title', 'd.grade', 'd.job_family_id', 'f.name as family', 'd.job_function_id', 'fn.name as func', 'd.default_grade_id', 'g.code as grade_code', 'd.career_level_id', 'l.code as level_code', (eb) => eb.selectFrom('employees as e').select((eb2) => eb2.fn.countAll<number>().as('n')).whereRef('e.designation_id', '=', 'd.id').where('e.deleted_at', 'is', null).as('employees')])
      .where('d.deleted_at', 'is', null).orderBy('d.title').execute()).map((d) => ({ id: d.id, code: d.code, title: d.title, grade: d.grade, jobFamilyId: d.job_family_id, jobFamilyName: d.family ?? null, jobFunctionId: d.job_function_id, jobFunctionName: d.func ?? null, defaultGradeId: d.default_grade_id, defaultGradeCode: d.grade_code ?? null, careerLevelId: d.career_level_id, careerLevelCode: d.level_code ?? null, employees: Number(d.employees ?? 0) })));
  ...
  r.get('/descriptions', { preHandler: requirePermission('jobs:read', 'employees:read:own'), schema: { tags: ['jobs'], summary: 'Job descriptions', querystring: z.object({ designationId: z.string().uuid().optional(), status: z.string().optional() }), response: { 200: z.array(jdOut) } } }, async (req) => { let q = jdQuery(); if (req.query.designationId) q = q.where('j.designation_id', '=', req.query.designationId); if (req.query.status) q = q.where('j.status', '=', req.query.status.toUpperCase()); return (await q.orderBy('d.title').execute()).map(jdMap); });
